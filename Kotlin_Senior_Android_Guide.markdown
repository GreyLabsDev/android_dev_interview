...
### Ограничения `inline` и случаи без подстановки

`inline` — директива Kotlin-компилятору при компиляции Kotlin call site, а не обещание, что JVM/ART оптимизирует
любой вызов. Не путайте её с JIT-inlining: JIT может встроить обычный маленький вызов в runtime или, наоборот,
не оптимизировать уже увеличенный bytecode при нехватке профиля или из-за размера метода.

- **Рекурсия запрещена.** `inline fun visit() = visit()` не скомпилируется: подстановка не имеет конечного размера.
- **Виртуальные функции не inline.** `inline` нельзя сочетать с `open`, `abstract` или `override`: для полиморфного
    вызова реализация выбирается в runtime, а тело inline должно быть известно в call site. Inline member-функция должна
    быть final.
- **`noinline`-параметры не подставляются.** Для такой лямбды создаётся объект функции; её можно сохранить, вернуть
    или передать в обычный callback. Это осознанный компромисс, а не ошибка оптимизации.
- **Лямбда, ушедшая в не-inline контекст, не может оставаться inline.** Если параметр нужно передать в `Runnable`,
    сохранить в поле или отправить в другой не-inline API, помечайте его `noinline`; иначе компилятор выдаст ошибку.
- **Вызовы из Java не встраиваются в Java-код.** Для Java inline-функция выглядит как обычный сгенерированный метод,
    поэтому преимущества подстановки есть только у Kotlin call site.
- **Слишком маленькая функция может получить warning.** Компилятор предупреждает об insignificant performance impact,
    когда inline не даёт практической выгоды. Не подавляйте его автоматически: чаще лучше убрать `inline`.

Компилятор всё равно генерирует обычный метод для JVM-совместимости и рефлексии; Kotlin-вызовы обычно получают
подставленное тело. Поэтому `inline` выбирают ради lambda-параметров, `reified` и control flow, а не как
универсальную микрооптимизацию.

## 5.4. `noinline` и `crossinline`

- `noinline` запрещает встраивание конкретной лямбды: её можно сохранить или передать как значение.
- `crossinline` **не** запрещает встраивание: он запрещает только non-local `return`, когда лямбда вызывается не
    напрямую, например внутри другого callback или `Runnable`.
...
В `runLater` тело лямбды может быть подставлено внутрь созданного callback, но `return` из неё не может вернуть
управление из внешней функции: callback выполнится позже. В `cache` объект `factory` необходим, потому что функция
возвращает его как значение.
...
Поэтому внутри функции доступны `T::class`, `T::class.java`, проверки `value is T` и создание объектов через
reflection API, принимающий `Class<T>` или `KClass<T>`. Тип известен именно в Kotlin call site, где компилятор
подставил тело; передать `T` дальше как полноценный runtime generic-аргумент всё равно нельзя.

`reified` доступен только у type parameter `inline`-функции. Он не применяется к классу, свойству или обычной
функции: для них передавайте `KClass<T>`, `Class<T>` либо type token явно. Это не отменяет erasure вложенных
generic-аргументов: проверка `value is List<String>` по-прежнему ненадёжна; можно проверить только `value is List<*>`.
...
## 8.1. Иерархия API и variance

На JVM `Iterable<T>` даёт только итерацию; `Collection<T>` добавляет `size`, `contains` и `isEmpty`; от него
наследуются `List<T>` и `Set<T>`. `Map<K, V>` стоит отдельно: это не `Collection`, а его `entries`, `keys` и
`values` являются collection-like представлениями. У каждого read-only интерфейса есть mutable-пара: `MutableList`,
`MutableSet`, `MutableMap`.

Read-only коллекции ковариантны: `List<Dog>` можно передать как `List<Animal>`, потому что через такую ссылку нельзя
добавить `Cat`. Mutable-коллекции инвариантны: `MutableList<Dog>` нельзя выдать за `MutableList<Animal>`. Для `Map`
это относится и к ключу, и к значению: read-only `Map` только читает их, mutable `Map` принимает их в `put`.

## 8.2. Read-only коллекция — это immutable коллекция?
...
Фабрики `listOf`, `setOf`, `mapOf` возвращают read-only типы; `mutableListOf`, `mutableSetOf`, `mutableMapOf` —
mutable. Не полагайтесь на конкретный runtime-класс, особенно для пустых и одноэлементных коллекций: stdlib вправе
использовать специализированные реализации. В прикладном коде выбирайте интерфейс по контракту, а не по результату
`javaClass`.

## 8.3. Что под капотом на JVM

Типичные JVM-реализации:

- `mutableListOf()` обычно создаёт `ArrayList`: непрерывный массив ссылок, быстрый `get(index)` O(1), хорошая cache
    locality; вставка или удаление в начале/середине сдвигает хвост O(n), увеличение capacity иногда копирует массив O(n).
- `ArrayDeque` хранит элементы в кольцевом буфере и подходит для очереди/стека: добавление и снятие с концов
    амортизированно O(1). Для очереди он обычно лучше `LinkedList`.
- `hashSetOf()` и `hashMapOf()` на JVM обычно используют hash-структуры; Kotlin-фабрики сохраняют порядок передачи
    элементов, поэтому типичная реализация — `LinkedHashSet`/`LinkedHashMap`. Поиск, вставка и удаление в среднем O(1),
    но зависят от корректных и стабильных `equals`/`hashCode`.
- `TreeSet`/`TreeMap` из Java поддерживают упорядоченность по comparator и дают O(log n) на поиск/вставку/удаление.
    Они нужны для range queries и сортированного обхода, а не как замена `HashMap` по умолчанию.
- `sorted`, `sortedBy` и `toSortedSet` материализуют вход и обычно имеют стоимость O(n log n); `distinct` хранит
    множество увиденных значений, то есть использует O(n) дополнительной памяти в худшем случае.

Это характеристики распространённых реализаций, а не обещание интерфейса `List`/`Set`/`Map`. Если алгоритму
нужен индексный доступ, очередь или упорядоченный поиск, выразите это конкретным типом либо API-контрактом.

## 8.4. Базовые операции, аллокации и итераторы

Операторы `+`/`-` над read-only коллекциями не меняют исходную коллекцию: они создают новую. В цикле это легко
превращается в O(n^2) и поток мусора:

```kotlin
val result = mutableListOf<Item>()
for (item in source) {
        if (item.isValid) result += item       // изменяет один ArrayList
}

val filtered = source.filter { it.isValid } // создаёт новую коллекцию один раз
```

`map`, `filter`, `associate`, `groupBy` и `partition` для `Iterable` тоже обычно создают результирующие коллекции.
Для накопления в уже выделенную цель применяйте `mapTo`, `filterTo` и `associateTo`; для построения read-only результата
удобен `buildList { ... }`. Копии поверхностны: новый список не копирует его элементы.

### Обход и побочные эффекты

- `forEach` вызывает действие для каждого элемента; `forEachIndexed` также передаёт индекс. Оба подходят для
    побочного эффекта, но не для преобразования данных.
- `for` обычно читается проще, когда нужны `break`, `continue` или сложный control flow. В `forEach` нет обычного
    `break`; `return@forEach` пропускает только текущую итерацию. Non-local `return` возможен только из inline-лямбды
    внутри именованной функции и редко делает код понятнее.
- `onEach` выполняет действие и возвращает исходный receiver, поэтому годится для логирования в цепочке,
    но не должен менять элементы, от которых зависит последующая логика.

```kotlin
val visibleIds = users
        .onEach { analytics.logViewed(it.id) }
        .filter { it.isVisible }
        .map { it.id }
```

### Преобразование, фильтрация и выборка

- `map`/`mapIndexed` создают один результат на элемент; `mapNotNull` одновременно преобразует и отбрасывает `null`.
- `filter`, `filterNot`, `filterNotNull` и `filterIsInstance<T>()` создают список подходящих элементов.
    `filterIsInstance` особенно удобен для sealed hierarchy или данных с небезопасной границы.
- `flatMap` преобразует каждый элемент в `Iterable`/`Sequence` и раскрывает один уровень вложенности;
    `flatten` только раскрывает уже вложенные коллекции. Оба могут создать много объектов.
- `distinct` и `distinctBy` оставляют первое вхождение каждого значения/ключа и держат set уже встреченных ключей.
- `sorted`, `sortedBy`, `sortedWith`, `sortedDescending` создают новый отсортированный результат; для изменения
    mutable-коллекции на месте существуют `sort`, `sortBy`, `sortWith`.
- `take`, `takeLast`, `drop`, `dropLast`, `chunked` и `windowed` создают срезы, чанки или скользящие окна. У `windowed`
    размер результата растёт вместе с числом окон; это важно для больших списков.

### Поиск и проверки без лишней работы

`first`, `firstOrNull`, `last`, `lastOrNull`, `find` (синоним `firstOrNull(predicate)`), `any`, `all`, `none` и
`indexOfFirst` обходят только до получения ответа, то есть short-circuit. `first` бросает `NoSuchElementException`,
если подходящего элемента нет; в прикладном коде чаще честнее `firstOrNull` с явной обработкой `null`.

`single`/`singleOrNull` выражают более сильный контракт: подходящий элемент должен быть **ровно один**. `single`
бросит исключение и при нуле, и при двух элементах; `singleOrNull` вернёт `null` в обоих случаях. Не заменяйте ими
`first`, если дубликаты допустимы бизнес-логикой.

### Агрегация

- `count`, `sum`, `sumOf`, `minOrNull`, `maxOrNull`, `minByOrNull`, `maxByOrNull` возвращают одно значение;
    `count { predicate }` не создаёт промежуточный `filter`-список.
- `fold(initial) { acc, item -> ... }` всегда использует переданный initial и безопасен для пустой коллекции.
- `reduce { acc, item -> ... }` берёт первый элемент как initial, поэтому бросает исключение для пустой коллекции;
    `reduceOrNull` возвращает `null`.
- `runningFold`/`runningReduce` возвращают все промежуточные аккумуляторы и потому потребляют O(n) дополнительной памяти.

```kotlin
val total = orders.fold(0L) { sum, order -> sum + order.priceCents }
val byStatus = orders.count { it.status == Status.PAID }
```

### Индексирование и группировка

`associate` создаёт `Map` из пар ключ-значение, `associateBy` индексирует элемент по ключу, `associateWith` вычисляет
значение для каждого элемента-ключа. При одинаковом ключе все три оставляют **последнее** значение; если дубликат —
ошибка домена, проверяйте его явно, а не теряйте данные молча.

`groupBy` создаёт `Map<K, List<V>>`: удобно и читаемо, но материализует список для каждой группы. Когда нужны только
счётчики или агрегация, `groupingBy { key }.eachCount()`/`fold()`/`reduce()` обычно экономнее, потому что не обязаны
создавать промежуточные списки групп.

```kotlin
val userById: Map<UserId, User> = users.associateBy { it.id }
val paidCountByCurrency: Map<Currency, Int> = orders
...
- `zip` соединяет элементы двух коллекций попарно и останавливается на более короткой; `unzip` разделяет пары на два
    списка.
- `plus`/`minus` возвращают новую коллекцию; `plus` не изменяет receiver.
- `partition` всегда создаёт **два** списка: подходящие и неподходящие. Если нужен только один из них, `filter` часто
    дешевле и яснее.

`for (item in items)` вызывает `iterator()`. У mutable-коллекции безопасно удалять текущий элемент только через
`MutableIterator.remove()` или `removeAll { predicate }`, а не через `items.remove(item)` во время обхода. Типичные
JVM-коллекции fail-fast и могут бросить `ConcurrentModificationException`, но это диагностика ошибки, а не механизм
потокобезопасности. Для доступа из нескольких потоков нужны ownership, синхронизация либо concurrent-коллекция,
выбранная под конкретную семантику.

## 8.5. Kotlin-коллекции и аналоги в Java

На JVM Kotlin `List`/`MutableList`, `Set`/`MutableSet` и `Map`/`MutableMap` в bytecode в основном отображаются на
`java.util.List`, `java.util.Set` и `java.util.Map`. У Java нет отдельного runtime-интерфейса для Kotlin read-only:
различие `List` и `MutableList` проверяется Kotlin-компилятором и metadata, а Java-код может попытаться изменить
полученный `List`. Успех зависит от конкретной реализации: `ArrayList` изменится, а immutable/unmodifiable объект
бросит `UnsupportedOperationException`.

В обратную сторону `java.util.List<String>` приходит в Kotlin как platform type: неизвестны и nullability элементов,
и фактическая изменяемость. Нельзя считать, что Java API вернул безопасный `List<String>` или что его нельзя менять.
Нормализуйте границу: валидируйте `null`, при необходимости копируйте `toList()`/`toMutableList()` и отдавайте наружу
наиболее узкий контракт. Java `Collections.unmodifiableList` создаёт только запрещающую мутацию обёртку над исходным
списком, а `List.of` (Java 9+) создаёт неизменяемую реализацию; оба варианта отличаются от Kotlin `List` как типа.

В Kotlin `Array<Int>` содержит boxed `Integer`, в то время как `IntArray` — примитивный `int[]`; Java-эквивалент
`int[]` также не является `List<Integer>`. На Android для больших данных и hot path это заметнее, чем косметический
выбор между `List` и `MutableList`.

## 8.6. `Iterable` против `Sequence`
...
## 8.8. `map`, `flatMap`, `flatten`, `mapNotNull`
...
## 8.9. Массивы и примитивные массивы
